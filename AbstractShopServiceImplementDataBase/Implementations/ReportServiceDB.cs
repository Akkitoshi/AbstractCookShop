...
namespace AbstractCookShopServiceImplementDataBase.Implementations
{
    public class ReportServiceDB : IReportService
    {
        private AbstractCookShopDbContext context;
        public ReportServiceDB(AbstractCookShopDbContext context)
        {
            this.context = context;
        }
        public void SaveCookPrice(ReportBindingModel model)
        {
            ...
            try
            {
               ...
               missing);
                ...
                var Cooks = context.Cooks.ToList();
                //создаем таблицу
                var paragraphTable = document.Paragraphs.Add(Type.Missing);
                var rangeTable = paragraphTable.Range;
                var table = document.Tables.Add(rangeTable, Cooks.Count, 2, ref
               missing, ref missing);
                ...
                {
                    table.Cell(i + 1, 1).Range.Text = Cooks[i].CookName;
                    table.Cell(i + 1, 2).Range.Text = Cooks[i].Price.ToString();
                }
        ...
        public List<StocksLoadViewModel> GetStocksLoad()
        {
           ...
           .GroupJoin(
            ...
            context.StockIngridientss
            .Include(r => r.Ingridients)
           .ToList(),
            stock => stock,
           stockIngridients => stockIngridients.SStock,
           (stock, stockCompList) =>
            new StocksLoadViewModel
            {
                StockName = stock.SStockName,
                TotalCount = stockCompList.Sum(r => r.Count),
                Ingridientss = stockCompList.Select(r => new Tuple<string,
    int>(r.Ingridients.IngridientsName, r.Count))
            })
            .ToList();
        }
                        ...
                        if (elem.Ingridientss.Count() > 0)
                        {
                           ...
                           Microsoft.Office.Interop.Excel.XlBorderWeight.xlThin;
                            ...
                            Microsoft.Office.Interop.Excel.XlColorIndex.xlColorIndexAutomatic,
                             1);
                            foreach (var listElem in elem.Ingridientss)
                            {
 ...
 excel.Workbooks[1].Save();
        ...
        public List<SClientOrdersModel> GetClientOrders(ReportBindingModel model)
        {
            return context.SOrders
            .Include(rec => rec.SClient)
           .Include(rec => rec.Cook)
        ...
        public void SaveClientOrders(ReportBindingModel model)
        {
           ...
           iTextSharp.text.Font.BOLD);
            ...
            table.AddCell(new PdfPCell(new Phrase("Блюдо", fontForCellBold))
            {
                HorizontalAlignment = Element.ALIGN_CENTER
            });
            ...
            for (int i = 0; i < list.Count; i++)
            {
                ...
                table.AddCell(cell);
                cell = new PdfPCell(new Phrase(list[i].CookName, fontForCells));
                table.AddCell(cell);
